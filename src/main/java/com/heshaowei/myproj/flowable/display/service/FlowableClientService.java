...
public class FlowableClientService {
    ...
    public String getAuthenticationToken(ServerConfig serverConfig, String clientId, String clientSecret){
        URIBuilder uriBuilder = this.createUriBuilder(serverConfig.getServerAddress() + ":" + serverConfig.getPort() + "/oauth/token");
        uriBuilder.addParameter("username", serverConfig.getUserName());
        uriBuilder.addParameter("password", serverConfig.getPassword());
        uriBuilder.addParameter("client_id", clientId);
        uriBuilder.addParameter("client_secret", clientSecret);
        uriBuilder.addParameter("grant_type", "password");

        HttpPost post = null;
        try {
            post = new HttpPost(uriBuilder.build().toString());
            JsonNode node = this.executeRequest(post);
    ...
    public CloseableHttpClient getHttpClient(ServerConfig serverConfig) {
        ...
        if(null != serverConfig && null != serverConfig.getUserName() && null != serverConfig.getPassword()) {
            CredentialsProvider credentialsProvider = new BasicCredentialsProvider();
            credentialsProvider.setCredentials(AuthScope.ANY, new UsernamePasswordCredentials(serverConfig.getUserName(), serverConfig.getPassword()));
            ...
            if (this.preemptiveBasicAuthentication) {
                ...
                String authenticationToken = getAuthenticationToken(serverConfig, "lucky", "123456");
                httpClientBuilder.setDefaultHeaders(Collections.singletonList(new BasicHeader("Authorization", authenticationToken)));
            }
        }
    ...
    public JsonNode executeRequest(HttpUriRequest request) {
        return this.executeRequest(request, null, 200);
    }
